...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box = label.box
            box_label = tools.compute_box_corners(box.center_x, box.center_y, box.width, box.length, box.heading)
            ...
            for detection in detections:
                ...
                distance_x = np.array(box.center_x - x).item()
                distance_y = np.array(box.center_y - y).item()
                distance_z = np.array(box.center_z - z).item()
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                
                polygon_label = Polygon(box_label)
                polygon_detection = Polygon(box_detection)
                intersection = polygon_label.intersection(polygon_detection).area
                union = polygon_label.union(polygon_detection).area
...
def compute_performance_stats(det_performance_all):
    ...
    positives = sum(pos_negs_array[:, 0])
